...
# create file handler which every time rewrite logs
fh = logging.FileHandler(LOGFILE, mode='w')
fh.setLevel(logging.DEBUG)
# create console handler
ch = logging.StreamHandler()
ch.setLevel(logging.DEBUG)
logging.basicConfig(level=logging.DEBUG, handlers=[fh, ch], format="%(asctime)s: %(levelname)s - %(funcName)s: %(message)s")
